...
$(document).on("mobileinit", function createSimpleHandler() {
	...
	var simpleHandler = function( name, reverse, $to, $from ) {
		...
		toClasses = reverse ? ' slide-sim in reverse' : ' slide-sim in',
		fromClasses = reverse ? ' slide-sim out reverse' : ' slide-sim out';
		
		$from.addClass( fromClasses );
		
		$to.addClass( $.mobile.activePageClass + toClasses );
