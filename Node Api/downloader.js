...
log4js.addAppender(log4js.appenders.file('logs/downloader2.log'), 'ChildDownloader');
...
// Connect to MongoDB
mongoose.connect('mongodb://127.0.0.1:27017/demo-dev1');
...
function download(libNameFromResponse, localFile, remotePath, callback) {
    ...
    var version = remotePath.substring(remotePath.indexOf(libNameFromResponse + '/') + libNameFromResponse.length + 1, remotePath.indexOf('/' + localFile)).split('/');
            ...
            if (null == err && savedFile.length>0) {
                ...
                chunkStream.on('end', function (stats) {
                    var promisedSavingVersion = new Promise(function (resolve, reject) {
                        ...
                        generateAST(fileContent, function (errast, AST) {
                            ...
                            if (null == errast) {
                                ...
                                writeStream.on('close', function (ASTfile) {
                                    ...
                                    ee.emit("saveVersionWithASTEvent", AST, savedFile, libraryObjID, ASTfile._id, HashSet);
                                    resolve();
                                });
...
ee.on("saveVersionWithASTEvent", function (AST, savedFile, libraryObjID, ASTfileID, HashSet) {


    var funcDeclarationHashSet = new Set("");
    var funcExpressionHashSet = new Set("");
    //var blockStatementHashSet = new Set("");
    //var callExpressionHashSet = new Set("");
    //var expressionStatementHashSet = new Set("");
    ...
    estraverse.traverse(AST, {
        enter: function (node, parent) {
            ...
            /* if (node.type == 'BlockStatement'){
             blockStatementHashSet.add(hash.digest(node));
             }
             else if (node.type == 'CallExpression'){
             callExpressionHashSet.add(hash.digest(node));
             }*/
            /*else if (node.type == 'ExpressionStatement'){
             expressionStatementHashSet.add(hash.digest(node));
             }*/
    ...
    var versionObj = new version({
        ...
        //blockStatementHashSet:blockStatementHashSet.toArray(),
        //callExpressionHashSet:callExpressionHashSet.toArray(),
        //expressionStatementHashSet:expressionStatementHashSet.toArray(),
    ...
    versionObj.save(function (err) {
        if (null != err) {
            logger.error("Error Saving Version Obj :" + err);
            //cb();
            //resolve();
        }
