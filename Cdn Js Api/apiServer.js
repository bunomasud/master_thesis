...
app.get('/libraries', function(req, res){
  ...
  function formatResults(fields, packages) {
    ...
    // format the results including optional `fields`
  function formatFieldResults(fields, packages) {
	console.log(req.query.search);
	
    return _.filter(packages, function (package) {
	  if(req.query.search==package.name){
  ...
  if (req.query.search) {
    var searchParams = {
	  ...
	  synonyms: false,
	  minWordSizefor1Typo:1000,
	  minWordSizefor2Typos:1000,
	  minProximity :1,
      typoTolerance: false,
	  hitsPerPage: 1000,
	  ignorePlurals :true
    ...
    algoliaIndex.search(req.query.search, searchParams, function(error, content) {
      ...
      var json = {
          results: formatFieldResults(fields, results),
